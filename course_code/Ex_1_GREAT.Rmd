...
peaks <- read.table("data/ex1/cluster30_peaks.bed", sep = "\t", header = FALSE) %>%
...
tss_df <- read.table("data/ex1/clodip_v4_TSS.bed", sep="\t", header = FALSE)
...
```{r}
scaf_len <- read.table("data/ex1/genome_CADEPI01_2023_scaffold_lengths.tbl",sep="\t", header = FALSE)
...
go<- read.table("data/ex1/clodip_v4_GO_long.txt", sep=" ", header = TRUE)
head(go, 5)
