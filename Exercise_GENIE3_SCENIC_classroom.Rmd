...
We will first generate a GRN based on the expression
...
```{r}
exprMat_dense <- readRDS("data/exprMat_nymph3_eye_SCENIC.rds")
meta          <- readRDS("data/meta_nymph3_eye_SCENIC.rds")
umap          <- readRDS("data/umap_nymph3_eye_SCENIC.rds")
...
regulonAUC     <- AUCell_calcAUC(regulons, cells_rankings, nCores = 2)
...
We identify specific transcription factors associated with known biological processes—such as neurogenesis, retinal development and sexual differentiation—based on literature and annotation. These TFs form a curated set of candidate regulators.
...
Let's take a look to the appereance of these genes in a Heatmap.
